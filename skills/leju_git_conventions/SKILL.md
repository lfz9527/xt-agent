...
在生成 commit message 之后、执行 `git commit` 之前，**必须**让用户亲眼看到完整的提交信息并确认。使用 AskUserQuestion 工具让用户确认，**提问时必须在内容中完整列出 commit message 本体（主题行 + 要点列表）**，再给出选项：
...
- **需要修改** → 用户选择"需要修改"，然后根据用户要求调整 commit message，调整后**再次完整列出新的 commit message** 重复确认，直到用户满意为止
- **取消提交** → 用户选择"取消"，终止提交流程，不执行 commit

确认时仅展示完整的 commit message（主题行 + 要点列表），无需列出提交文件或排除文件。

此确认步骤**不可跳过** —— 即使用户说"直接提交"或"不用确认"，也必须执行；提交信息本体**必须逐字列出供用户查看**，严禁只弹选项、不展示内容。
...
Closes [关联 issue 的完整链接 URL]
```

缺少任一字段，或使用关键词但不带 `字段名：` 格式均为无效。

`Closes` 字段**必须使用关联 issue 的完整链接 URL**（如 `Closes https://lejuhub.com/xxx/project/issues/549`），禁止只写 issue 号。

- 分支名含 issue 号时（如 `lfz/bug/549_login_white_screen` → issue 549），根据项目 GitLab 地址拼出完整链接
- **没有链接/不确定链接时，向用户询问获取完整 URL，不得自行猜测**
- 用户明确说明"没有关联 issue"时，**不展示 `Closes` 字段**
...
生成 MR 描述后、创建/提交 MR 之前，**必须**让用户亲眼看到完整的 MR 标题 + 描述并确认。使用 AskUserQuestion 工具让用户确认，**提问时必须在内容中完整列出 MR 标题和全部结构化字段**，再给出选项：
...
问题现象：登录页在弱网环境下刷新出现白屏

问题原因：token 异步获取未等待完成即渲染

修复方案：将 token 获取改为 await 后再渲染

引入问题：commit: abc1234 author:lfz

Closes https://lejuhub.com/lfz9527/xt-agent/issues/549
```

- **确认无误** → 用户选择"确认无误"后，继续创建/提交 MR
- **需要修改** → 用户选择"需要修改"，根据用户要求调整描述，调整后**再次完整列出新的 MR 标题和描述**重复确认，直到用户满意为止
- **取消** → 用户选择"取消"，终止 MR 创建流程

此确认步骤**不可跳过** —— 即使用户说"直接创建"或"不用确认"，也必须执行；MR 标题和描述本体**必须逐字列出供用户查看**，严禁只弹选项、不展示内容。
...
9. 将完整的 MR 标题 + 描述逐字列出，使用 AskUserQuestion 让用户确认（见下方「功能类 MR 描述确认」）
```

`Closes` 字段**必须使用关联 issue 的完整链接 URL**（如 `Closes https://lejuhub.com/xxx/project/issues/549`），禁止只写 issue 号。

- 分支名含 issue 号时（如 `lfz/feat/549_dance_base` → issue 549），根据项目 GitLab 地址拼出完整链接
- **没有链接/不确定链接时，向用户询问获取完整 URL，不得自行猜测**
- 用户明确说明"没有关联 issue"时，**不展示 `Closes` 字段**
...
需求目标：实现版本管理设置面板，支持版本号配置与灰度发布

需求原因：线上版本回滚依赖手工操作，耗时且易出错

实现方案：新增 SettingsDrawer 组件，抽离配置逻辑至独立 store
...
| 修复类 MR | 必须包含：问题现象、问题原因、解决方法、引入来源、责任人；`Closes` 用完整链接，无链接先询问，明确没有则不展示；确认时完整列出标题+描述 |
| 功能类 MR | 必须包含：类型、需求目标、需求原因、实现方案；`Closes` 用完整链接，无链接先询问，明确没有则不展示；确认时完整列出标题+描述 |
