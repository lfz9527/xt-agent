...
export class RunAuditTimeline {
  ...
  mutation(entry: MutationJournalEntry, policyRevision: number): void {
    this.append(entry.runId, policyRevision, 'RESOURCE_MUTATION', entry as unknown as Record<string, unknown>);
  }
  ...
  private append(runId: string, policyRevision: number, type: RuntimeAuditEvent['type'], payload: Record<string, unknown>): void {
    ...
    this.audit.append({ eventId: createRuntimeEventId(type.toLowerCase()), runId, type, at: new Date().toISOString(), policyRevision, payload: { timelineKind: type.toLowerCase(), ...payload } });
  }
}
