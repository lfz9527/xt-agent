...
export interface LoopRuntimeState { runId: string; status: string; policyRevision: number; snapshot: PolicySnapshot; facts: RuntimeFacts; gitBaseline?: GitBaseline; expectedWorktreeFingerprint?: string; }
...
export class LoopRuntimeKernel {
  ...
  transition(to: string): void {
    if (!this.resourceLock) throw new Error('[LOOP_BLOCKED] state lock is required for transition'); const initialState = this.stateStore.read(); const lockResource = this.stateLockResource(initialState.runId);
    this.resourceLock.acquire(lockResource, initialState.runId);
    try {
      this.resourceLock.assertOwned(lockResource, initialState.runId); const state = this.stateStore.read(); if (state.runId !== initialState.runId) throw new Error('[LOOP_BLOCKED] runtime run identity changed while acquiring state lock');
      ...
      const from = state.status; this.stateStore.write({ ...state, status: to }); if (this.auditTimeline) this.auditTimeline.state(state.runId, state.policyRevision, from, to); else this.audit?.append({ eventId: createRuntimeEventId('transition'), runId: state.runId, type: 'STATE_TRANSITION', at: new Date().toISOString(), policyRevision: state.policyRevision, payload: { from, to } });
